...
interface IWebsocketRoute<Action: IWebsocketAction> {
    ...
    val path: String
    fun execute(action: Action, user: User): IWebsocketNotification?
}

class DefaultWebsocketRoute<A: IWebsocketAction>(
    override val path: String,
    override val actionType: KClass<A>,
    val block: IWebsocketRoute<A>.(action: A, user: User) -> IWebsocketNotification?
): IWebsocketRoute<A> {

    override fun execute(action: A, user: User): IWebsocketNotification? {
